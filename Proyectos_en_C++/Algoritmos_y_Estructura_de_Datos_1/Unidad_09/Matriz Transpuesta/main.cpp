/* Trabajaremos con Punteros a Punteros

Realice un programa que lea una matriz dinámica de NxM y cree su
matriz traspuesta.
...
**puntero_matriz -> *puntero_fila [int][int]
    ...
    *(elemento+j) // elemento [j]
    *(*(puntero_matriz+i)+j); // puntero_matrix[i][j]
...
void pedirDatos(){
	...
	for(int i=0;i<nFilas;i++){
		for(int j=0;j<nCol;j++){
			...
			cin>>*(*(puntero_matriz+i)+j); // puntero_matrix[i][j]
		}
	}
}
